...
\begin{minipage}{\columnwidth}
  ...
  \vmspace
  \begin{center}
    $p(x) = \dfrac{1}{\sigma \sqrt{2\pi}}\,\mathrm{exp}\left[-\dfrac{(x -
...
\begin{minipage}{\columnwidth}
  \textbf{Multivariate Gaussian distribution} for dimension $k$ with covariance
  matrix $\Sigma$ (positive semidefinite, symmetric):
  \vmspace
  \begin{center}
    $
    p(x)
...
\begin{minipage}{\columnwidth}
  ...
  \item Probabilistic motion model: $x_t = f(x_{t-1}, u_t)$.
  \item Probabilistic measurement model: $z_t = h(x_t, M)$.
  \end{compactenum}
\end{minipage}
...
\begin{minipage}{\columnwidth}
  ...
  \begin{compactitem}
  \item \textbf{Position tracking:} $bel(x_0)$ is \textbf{Diract delta}
    function.
...
\begin{minipage}{\columnwidth}
  \includegraphics[width=\columnwidth]{img/8_ActSee.png}

  \vspace{-4pt}

  \begin{compactenum}
  \item \textbf{ACT:} Motion estimated by odometry ($x_3$).
  \item \textbf{SEE:} Observation of landmark with uncertainty ($x_3^\prime$).
  \item \textbf{Belief update:} Information fusion ($x_3^{\prime\prime}$).
  \end{compactenum}
\end{minipage}
...
\begin{minipage}{\columnwidth}
  ...
  \begin{center}
    ...
    bel(x_t)
    =
    p(x_t|x_0, u_t \cdots u_0, z_t \cdots z_0)
    =
...
\begin{minipage}{\columnwidth}
  Algorithm for \textbf{Markov localization}, based on map $M$:
